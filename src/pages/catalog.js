...
function extractIngredients(drink) {
    ...
    for (let i = 1; i <= 15; i++) {
        ...
        if (ing && ing.trim()) list.push(measure ? `${measure.trim()} ${ing.trim()}` : ing.trim());
    }
    return list;
}
...
const renderGrid = () => {
    ...
    if (filtered.length === 0) return `<p class="catalog__empty">Рецепты не найдены</p>`;

    const visible = expanded ? filtered : filtered.slice(0, PAGE_SIZE);
    const hasMore = filtered.length > PAGE_SIZE;

    return visible.map(r => r.renderCatalogCard()).join('') + (hasMore ? `
    ...
    <div class="screen screen--catalog">
        <div class="catalog">
            ...
            <div class="catalog__grid" id="catalog-grid">
                ${ALL_RECIPES.slice(0, PAGE_SIZE).map(r => r.renderCatalogCard()).join('')}
                <button class="catalog__show-more" id="catalog-show-more">
        ...
        const details = await Promise.all(
            drinks.map(d =>
                fetch(`${COCKTAIL_API}/lookup.php?i=${d.idDrink}`)
                    .then(r => r.json()).then(j => j.drinks?.[0]).catch(() => null)
            )
        );
        const apiRecipes = details.filter(Boolean).map((drink, i) =>
            CoffeeRecipe.fromJSON({
                id: `api-${drink.idDrink}`, name: drink.strDrink,
                desc: (drink.strInstructions ?? '').slice(0, 65).trim() + '…',
                time: '5 мин', icon: ICONS[i % ICONS.length], category: 'espresso',
                ingredients: extractIngredients(drink), steps: parseSteps(drink.strInstructions),
                fullDesc: drink.strInstructions ?? '', image: drink.strDrinkThumb ?? null,
            })
        );
